...
$stmt = $conn->prepare("SELECT recipe_id, recipe_title, recipe_image FROM favorites WHERE user_id = :user ORDER BY created_at DESC"); //Select favorite recipes for the user and order by creation date descending which means most recent first
