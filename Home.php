...
try {
  $pdo = new PDO('mysql:host=locationhost;dbname=your_db_name', 'your_db_user', 'your_db_pass');
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

  $userId = $_SESSION['user_id']; // Assuming this set in checkSession.php

  // Fetch user stats
  $stmt = $pdo->prepare("SELECT COALESCE((SELECT COUNT(*) FROM favorites WHERE user_id = ?), 0) as favorites_count,
  COALESCE((SELECT COUNT(*) FROM meal_plans WHERE user_id = ? AND week_start >= CURDATE() - INTERVAL 7 DAY), 0) as active_plans
  FROM dual");
  ...
  <body>
    ...
    <main class="page-container">
      <section class="hero">
        ...
        <p>You have <?php echo $userStats['favorites_count']; ?> favorite recipes and <?php echo $userStats['active_plans']; ?> active meal plans.</p>
      ...
      <section class="home-grid">
        <article class="home-card">
          ...
          <a href="meal-planner.php" class="link">Open Meal Planner (<?php echo $userStats['active_plans']; ?> active)</a>
        </article>
      ...
      <section class="stats-grid">
        ...
        <article class="home-card">
          <h2>Quick Actions</h2>
          <p>Jump right into planning your next meals.</p>
