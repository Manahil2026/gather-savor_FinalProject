...
if($_SERVER['REQUEST_METHOD'] !== 'POST'){ // Check if the request mehod is not POST
    redirect_with_msg('../../register.php', 'error', 'Invalid request method.'); // If it's not POST redirect to the register page with an error message
}
...
if(empty($name) || empty($email) || empty($password) || empty($confirm_password)){ // Check if any of the fileds are empty
    redirect_with_msg('../../register.php', 'error', 'All fields are required.'); // If any field is empty redirect to the register page with an error message
}
...
if(!filter_var($email, FILTER_VALIDATE_EMAIL)){ // Validate the email format using PHP's filter_var function
    redirect_with_msg('/../register.php', 'error', 'Invalid email format.'); // If the email format is invalid redirect to the register page with an error message
}

if($password !== $confirm_password){ // Check to see if psswrd and confrim psswrd match
    redirect_with_msg('../../register.php', 'error', 'Passwords do not match.'); // Redirect to register page if they dont match
}

//Redirect to register page if password is less than 6 characters
...
try{
    ...
    redirect_with_msg('../../register.php', 'error', 'Server error:' . $e->getMessage()); // Catch any PDO exceptions (database errors) and redirect to register page with error message
}
